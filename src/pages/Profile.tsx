    ...
    <div className="flex flex-col h-full bg-arc-bg">
      ...
      <div className="flex-1 overflow-y-auto px-4 py-6 space-y-6">
        <div className="flex flex-col items-center gap-3">
          <div className="relative">
            <div className="w-20 h-20 rounded-3xl border border-arc-border bg-arc-card flex items-center justify-center text-white text-3xl font-black shadow-xl">
              {initial}
            </div>
            <div className="absolute -bottom-2 -right-2 bg-white border border-white rounded-full px-2 py-0.5 text-[10px] font-bold text-black shadow-lg">
              Lv {level}
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 mt-1">
            <span className="text-xs px-3 py-1 rounded-full bg-white text-black border border-white font-bold">
              Level {level}
            </span>
          ...
          <div className="grid grid-cols-2 gap-3">
            {badges.map((badge) => (
                ...
                className={`relative flex flex-col items-center gap-2 p-3 text-center transition-all group ${
                  badge.isUnlocked
                    ? 'bg-arc-card border-arc-borderEmphasis'
              ...
              >
                ...
                <div className={`w-10 h-10 rounded-xl flex items-center justify-center transition-transform group-hover:scale-110 ${
                  badge.isUnlocked ? 'bg-white text-black' : 'bg-arc-border text-arc-text-dim'
                }`}>
                  <Trophy size={20} />
                </div>
                <div className="space-y-0.5">
                  <p className={`text-[10px] font-bold uppercase tracking-wider ${badge.isUnlocked ? 'text-white' : 'text-arc-text-dim'}`}>
                    {badge.label}
                  </p>
      ...
      {isEditing && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <Card className="w-full max-w-sm p-5 space-y-4 shadow-2xl border-arc-borderEmphasis">
            ...
            <div className="space-y-3">
              ...
              <div className="space-y-1.5">
                ...
                <textarea
                  className="w-full bg-arc-bg border border-arc-border rounded-xl p-3 text-sm text-arc-text placeholder:text-arc-text-dim focus:outline-none focus:border-white/50 transition-colors min-h-[100px] resize-none"
      ...
      {toast && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 animate-in fade-in slide-in-from-bottom-4 duration-300">
          <div className="bg-arc-card border border-arc-borderEmphasis text-white px-4 py-2 rounded-full text-xs font-bold shadow-2xl backdrop-blur-md">
            {toast}
          </div>
        </div>
      )}
