...
4. Type `demo script` if you want Gogo to show a compact judging/screen-recording narration.
5. Type `portfolio` to show real wallet intelligence.
6. Type `who should I tip` to show the paired-agent policy, weekly budget, per-tip cap, and advisor reasoning.
7. Type `x402 demo`, review the 0.001 USDC quote, then tap **Pay & access** to unlock the protected resource.
8. Type `create a reminder to pay 0.001 USDC to 0xB87B6D1a56bB7942bd07b6B0e9540a63b3dA4365 every 1 hour` to create a recurring autonomous payment without relying on an AI provider.
9. Open Settings -> Your agent (paired) -> Scheduled autonomous payments -> History and show the completed ArcScan transaction.
